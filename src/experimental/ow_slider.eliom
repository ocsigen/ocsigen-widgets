...
let create ?(orientation = Horizontal)
  ...
  then failwith "Ew_slider.create initial_value have to be between 0. and 1.";
...
let start (slider, dragger, ori, value,
  ...
  let handle_one_event callback ev =
    let coord =
      let x, y =  Ow_slide_event.get_slide_coord 0 ev in
  ...
  Lwt.async (fun () -> Ow_slide_event.touch_or_mouse_slides dom_dragger
  ...
  Lwt.async (fun () -> Lwt_js_events.clicks dom_slider (fun ev _ ->
    let x, y =
      let x, y = Ow_event_tools.get_mouse_ev_coord ev in
      let x', y' = x - !ox_slider, y - !oy_slider in
      x' - margin, y' - margin
    in
    let _ = match ori with
